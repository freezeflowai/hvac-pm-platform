...
import { ArrowLeft, MapPin, Phone, Mail, Plus, Pencil, Trash2, Briefcase, FileText, Wrench, Package, AlertCircle, ChevronRight } from "lucide-react";
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                <label className="text-xs font-medium text-muted-foreground">PM Type</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-muted-foreground">Scheduled Months</label>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground">
                    Next PM: <span className="font-medium text-foreground">—</span>
            ...
            <div className="mt-4 flex justify-end">
              <Button size="sm" data-testid="button-generate-pm-job">
                Generate PM Job
              </Button>
            </div>
          </CardContent>
                ...
                <div key={eq.id} className="rounded-lg border p-3 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Serial: <span className="font-medium">{eq.serialNumber || "—"}</span>
                    </div>
                  </div>
                  <div className="mt-2 text-xs text-muted-foreground">
                    <div>Make/Model: {eq.manufacturer} {eq.modelNumber}</div>
                    <div>Warranty: {eq.warrantyExpiry ? format(new Date(eq.warrantyExpiry), "MMM dd, yyyy") : "—"}</div>
                  </div>
                </div>
              ))
            )}
        ...
        <Card>
          ...
          <CardContent>
            {pmParts.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No PM parts configured. Add filters, belts, and other recurring parts used during each PM visit.
              </p>
            ) : (
              <div className="space-y-2">
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between gap-2">
            <CardTitle className="text-sm font-semibold">Location Notes</CardTitle>
            {!isAddingNote && (
              <Button
                variant="ghost"
                size="sm"
          ...
          <CardContent className="space-y-3">
            ...
            {notes.length === 0 && !isAddingNote && (
              <p className="text-xs text-muted-foreground">
                No notes yet. Use notes to record location specific info, access rules, landlord details, etc.
              </p>
            )}
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2">
            {locationJobs.length === 0 ? (
              <p className="text-sm text-muted-foreground">No jobs yet for this location.</p>
            ) : (
              locationJobs.slice(0, 10).map((job) => (
                <div key={job.id} className="flex items-center justify-between rounded-lg border p-3 text-sm">
                  <div>
                    <div className="font-medium">#{job.jobNumber} • {job.summary}</div>
                    <div className="text-xs text-muted-foreground">
            ...
            {locationJobs.length > 10 && (
              <p className="text-xs text-muted-foreground">+ {locationJobs.length - 10} more jobs</p>
            )}
          </CardContent>
        </Card>
      </div>
